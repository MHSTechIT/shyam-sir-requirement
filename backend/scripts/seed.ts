...
const GROUPS = [
  { id: "wc", name: "Wellness", color: "#ff7043", order: 0 },
  { id: "vsl", name: "VSL", color: "#26c6da", order: 1 },
  { id: "col", name: "Collab", color: "#66bb6a", order: 2 },
  { id: "dev", name: "Dev", color: "#7c4dff", order: 3 },
  { id: "mkt", name: "Mktg", color: "#f06292", order: 4 },
  { id: "ops", name: "Ops", color: "#1e88e5", order: 5 },
];
...
async function main() {
  ...
  // Groups — upsert so this runs even when nodes already exist.
  for (const g of GROUPS) {
    await query(
      `INSERT INTO groups ("id","name","color","order") VALUES ($1,$2,$3,$4)
       ON CONFLICT ("id") DO NOTHING`,
      [g.id, g.name, g.color, g.order]
    );
  }
  console.log(`Ensured ${GROUPS.length} groups.`);

  const countRow = await query<{ count: string }>(`SELECT COUNT(*)::int AS count FROM nodes`);
  if (Number(countRow[0].count) > 0) {
    console.log(`Database already has ${countRow[0].count} nodes — skipping node seed.`);
    return;
  }
  ...
  for (const n of NODES) {
    await query(
      `INSERT INTO nodes
         ("id","badge","title","sub","color","project","status","x","y","view","collapsed","hc","clarity","updatedAt")
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'master',false,$10::jsonb,$11::jsonb,CURRENT_TIMESTAMP)`,
      [
        n.id, n.badge, n.title, n.sub, n.color, n.project, n.status, n.x, n.y,
        n.hc ? JSON.stringify(n.hc) : null, JSON.stringify(DEFAULT_CLARITY),
      ]
    );
  }
  for (const [from, to] of CONNECTIONS) {
    await query(
      `INSERT INTO connections ("id","fromNode","toNode","view") VALUES ($1,$2,$3,'master')`,
      [`conn_${from}_${to}`, from, to]
    );
  }
  await query(`INSERT INTO history ("action") VALUES ($1)`, ["Seeded initial org structure"]);
  console.log(`Seeded ${NODES.length} nodes and ${CONNECTIONS.length} connections.`);
}
