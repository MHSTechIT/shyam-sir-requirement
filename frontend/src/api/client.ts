...
export const api = {
  ...
  // ── groups (named color filters) ──
  createGroup: (name: string, color: string) =>
    req<Group>("/groups", {
      method: "POST",
      body: JSON.stringify({ name, color }),
    }),
  updateGroup: (id: string, patch: { name?: string; color?: string }) =>
    req<Group>(`/groups/${id}`, { method: "PATCH", body: JSON.stringify(patch) }),
  deleteGroup: (id: string) =>
    req<{ ok: true }>(`/groups/${id}`, { method: "DELETE" }),
