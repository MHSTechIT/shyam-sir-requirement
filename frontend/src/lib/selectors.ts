...
export function isNodeVisible(
  ...
  if (view === "master") {
    // A node is hidden only when its group's filter is explicitly off.
    // Nodes with no matching group (e.g. "shared") are always visible.
    if (filters[node.project] === false) return false;
  }
